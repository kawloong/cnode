...
#else
#define ERRLOG_IF1BRK(exp, n, format, ...) do{}while(0)
#define IFBREAK_N(...) do{}while(0)
#define LOGERROR(...) do{}while(0)
#endif

// 默认返回noblock sockfd
int Sock::create_listen(char* ip, int port, bool v6)
{
    char* ipaddr = ip;
    int s = -1;
    int ret = 0;
...
std::string Sock::sock_name(int fd, bool hasport /*= false*/, bool v6 /*= false*/)
{
    ...
    do
    {
        if (v6)
        {
            ...
            result = getsockname(fd, (sockaddr*)&addr6, &addr_len);
        ...
        else
        {
            ...
            result = getsockname(fd, (sockaddr*)&addr4, &addr_len);
            ERRLOG_IF1BRK(result, result, "SOCKNAME| err(%d)=%s", errno, strerror(errno));
