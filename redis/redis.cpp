...
int Redis::set(const char* key, const char* value)
{
    ...
    {
        string strcmd("set ");
        IFBREAK_N(NULL==key || NULL==value, ERR_PARAM_INPUT);
...
int Redis::hset(const char* key, const char* field, const char* value)
{
    ...
    {
        string strcmd("hset ");
        IFBREAK_N(NULL==key || NULL==field || NULL==value, ERR_PARAM_INPUT);
...
int Redis::hget(string& value, const char* key, const char* field)
{
    ...
    {
        string strcmd("hget ");
        IFBREAK_N(NULL==key || NULL==field, ERR_PARAM_INPUT);
...
int Redis::hmset(const char* key, const char* strhash)
{
    ...
    {
        string strcmd("hmset ");
        IFBREAK_N(NULL==key || NULL==strhash, ERR_PARAM_INPUT);
...
int Redis::reconnect( void )
{
    ...
    if (pserver)
    {
        ...
        if (ret)
        {
            LOGERROR("REDIS_OPT| msg=selectno fail| name=%s| ret=%d| err=%s",
                pRedisConf_->redisname, ret, pserver->errstr);
            return ret;
        }
    }
